...
public class SaveService {
    ...
    public SaveService(Activity activity, LandingScreenPresentor.OnComplete onComplete) {
        ...
        //notificationTools = new NotificationTools(activity);
    }

    public int onStartCommand(String pageUrl) {
    ...
    private class PageSaveTask implements Runnable {
        ...
        public void run() {
            try {
                ...
                pageSaver.getOptions().setUserAgent(sharedPreferences.getString("user_agent", activity.getResources().getStringArray(R.array.entries_list_preference)[1]));
                ...
                Log.e("destinationDirectory", destinationDirectory);
                ...
                new Database(activity).addToDatabase(newSavedPageDirectory.getPath() + File.separator, pageSaver.getPageTitle(), pageUrl);

                if (sharedPreferences.getBoolean("generate_saved_page_thumbnails", true)) {
                    Intent i = new Intent(activity, ScreenshotService.class);
                ...
                Book book = new Book();
                book.setBookURL(newSavedPageDirectory.getPath() + File.separator + "index.html");
                book.setBookName(/*"index.html"*/ pageSaver.getPageTitle());

                dbHelper.addBook(book);

                onComplete.onSuccess(book.getBookURL());
                ...
                Toast.makeText(activity, "SaveService Exception: " + e.getMessage(), Toast.LENGTH_LONG).show();
                e.printStackTrace();
            }
        }
